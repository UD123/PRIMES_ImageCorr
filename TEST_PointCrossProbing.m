function TEST_PointCrossProbing(hFig1, hFig2, Loc1,Loc2, MatchMtrx)
% TEST_PointCrossProbing  - probing between matching points
% on a figure
% Inputs:
%   hFig1       - scatter plot
...
% Ver	Date	 Who	Descr
%-----------------------------
% 06.00 04.08.13 UD     Two points side by side
...
axisLimX        = [min(min(Loc1(:,1),min(Loc2(:,1)) max(max(Loc1(:,1)),max(Loc2(:,1)))]*1.1;
axisLimY        = [min(min(Loc1(:,2),min(Loc2(:,2)) max(max(Loc1(:,2)),max(Loc2(:,2)))]*1.1;


% Init scatter figure
figure(hFig1),clf
hold on; 
plot(Loc1(:,1),Loc1(:,2),'oy');
text(Loc1(:,1)+1,Loc1(:,2),num2str((1:n1)'),'color','g');
xlim(axisLimX),ylim(axisLimY)
hold off;
...
% get plot 
%MatchMtrx       = MatchMtrx*diag(1./(eps+sum(MatchMtrx)));
[sv,si]         = max(MatchMtrx);
[svs,sis]       = sort(sv,'descend');
si              = si(sis(1:showMaxNum));
hold on;
plot(Loc1(si,1),Loc1(si,2),'.m','MarkerSize',12);
hold off;
...
si              = si(sis(1:showMaxNum));
hold on;
plot(Loc2(si,1),Loc2(si,2),'.m','MarkerSize',12);
hold off;
